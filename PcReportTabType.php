...
if (isset($_POST['SelectedType'])){
	...
	$SelectedType = strtoupper($_GET['SelectedTabs']);
}
...
if ((! isset($_POST['FromDate']) AND ! isset($_POST['ToDate'])) OR isset($_POST['SelectDifferentDate'])){
	...
	echo '<tr><td>' . _('Code Of Petty Cash Tab') . ":</td><td><select name='SelectedType'>";
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		echo $myrow['typetabcode'] . "'>" . $myrow['typetabdescription'];
	...
	$SQL = "SELECT * FROM pcashdetails
			LEFT JOIN pctabs
				ON pccashdetails.tabcode=pctabs.tabcode
			WHERE pctabs.typetabcode='".$SelectedType."'
...
if (DB_error_no($db)!=0){
	...
	$LeftOvers = $pdf->addTextWrap($Left_Margin+110,$YPos,70,$FontSize,$SelectedType);
	...
	$SqlTabs = "SELECT * , pctypetabs.typetabdescription
			FROM pcashdetails
			LEFT JOIN pctabs
	...
	echo '<tr><td>' . _('Type Of Tab') . '</td>
				<td>:</td>
				<td width=200>' . ''.$Tabs['typetabdescription'].'</td>'  .
