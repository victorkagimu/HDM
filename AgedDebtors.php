...
if (isset($_POST['PrintPDF']) or isset($_POST['PrintCSV'])
...
} else if (isset($_POST['PrintCSV'])){//the user clicked on print csv instead of print 
	function stripcomma($str) { //because we're using comma as a delimiter
		return str_replace(",", "", $str);
	}
			
	if (!file_exists($_SESSION['DatabaseName'] . '_' . 'AgedDebtors45_' . date('Y-m-d') . '.csv')){
		$Result = mkdir($_SESSION['DatabaseName'] . '_' . 'AgedDebtors_' . date('Y-m-d') . '.csv');
	}
	
	$filename = $_SESSION['reports_dir'] . '/'.$_SESSION['DatabaseName'] . '_' . 'AgedDebtors_' . date('Y-m-d') . '.csv'; //the name should be the same as that of the pdf
	
	$fp = fopen($filename,"w");
	
	//Formatting the header for the csv, there is need to get the system to put titles every so after n lines
	$line = stripcomma($_SESSION['CompanyRecord']['coyname']);
	fputs($fp, $line. "\n");
	
	
	$HeadingLine1 = 'Aged Customer Balances For Customers from' . ' ' . $_POST['FromCriteria'] . ' ' .  _('to') . ' ' . $_POST['ToCriteria'];
	$HeadingLine2 = 'And Trading in'. ' ' . $_POST['Currency'];
	fputs($fp, $HeadingLine1." \n");
	fputs($fp, $HeadingLine2." \n");
	if (trim($_POST['Salesman'])!=''){
		...
		$numHeads++;
		fputs($fp, stripcomma($HeadingLine3."\n"));
	}
	
	$DatePrintedString = _('Printed') . ': ' . Date("d M Y") ;
	fputs($fp, stripcomma($DatePrintedString)."\n");
	
	$line = "Customer,Balance,Current,Due Now".','.$_SESSION['PastDueDays1'] . 'Days Over'.','.$_SESSION['PastDueDays2'] . 'Days Over';
	fputs($fp, $line."\n");
	
	//start the processing of the system
	$TotBal=0;
	$TotCurr=0;
	$TotDue=0;
	$TotOD1=0;
	...
	while ($AgedAnalysis = DB_fetch_array($CustomerResult,$db)){
		...
		$LeftOvers = $pdf->addTextWrap(400,$YPos,60,$FontSize,$DisplayOverdue1,'right');
		$LeftOvers = $pdf->addTextWrap(460,$YPos,60,$FontSize,$DisplayOverdue2,'right');*/
		$line = stripcomma($AgedAnalysis['debtorno'] . ' - ' . $AgedAnalysis['name'] ). ', '.stripcomma($DisplayBalance).', '.stripcomma($DisplayCurrent).', '.stripcomma($DisplayDue).','.stripcomma($DisplayOverdue1).','.stripcomma($DisplayOverdue2);
		fputs($fp, $line . "\n");
		...
		//For a comma delimited file this may not be important
	
	
		if ($_POST['DetailedReport']=='Yes'){
	
			/*draw a line under the customer aged analysis*/ //Line not important for this purpose
			//$pdf->line($Page_Width-$Right_Margin, $YPos+10,$Left_Margin, $YPos+10);
			...
			while ($DetailTrans = DB_fetch_array($DetailResult)){
				...
				//$line = stripcomma($AgedAnalysis['debtorno'] . ' - ' . $AgedAnalysis['name'] . ', '.stripcomma($DisplayBalance).', '.stripcomma($DisplayCurrent).', '.$DisplayDue.','.$DisplayOverdue1.','.$DisplayOverdue2);
				 //print a new line
				...
				$LeftOvers = $pdf->addTextWrap(400,$YPos,60,$FontSize,$DisplayOverdue1,'right');
				$LeftOvers = $pdf->addTextWrap(460,$YPos,60,$FontSize,$DisplayOverdue2,'right');*/
				$line=stripcomma($DetailTrans['typename']).', '.stripcomma($DetailTrans['transno']).','.stripcomma($DisplayTranDate['trandate']).','.stripcomma($DisplayDue).','.stripcomma($DisplayCurrent).','.stripcomma($DisplayBalance).','.stripcomma($DisplayOverdue1).','.stripcomma($DisplayOverdue2);
				echo $line. "<br>";
				fputs($fp, $line. "\n");
				/*$YPos -=$line_height;
				if ($YPos < $Bottom_Margin + $line_height){
	...
	$LeftOvers = $pdf->addTextWrap(400,$YPos,60,$FontSize,$DisplayTotOverdue1,'right');
	$LeftOvers = $pdf->addTextWrap(460,$YPos,60,$FontSize,$DisplayTotOverdue2,'right');*/
	$line=stripcomma($DisplayTotBalance).','.stripcomma($DisplayTotBalance).','.stripcomma($DisplayTotDue).','.stripcomma($DisplayTotOverdue1).','.stripcomma($DisplayTotOverdue2);
	fputs($fp, $line. "\n");
	...
	fclose($fp);
	echo '<br /><div class="centre"><a href="' . $rootpath . '/' . $_SESSION['reports_dir'] . '/'.$_SESSION['DatabaseName'] . '_' . 'AgedDebtors_' . date('Y-m-d') . '.csv ">' . _('click here') . '</a> ' . _('to view the file') . '</div>';

}//end if it is to write a csv file instead
	
}else{ /*The option to print PDF or print csv was not hit */
	...
	/*if $FromCriteria is not set then show a form to allow input	*/
		...
		echo '</select></td>
			...
			<div class="centre">
				<input tabindex="7" type="submit" name="PrintCSV" value="' . _('Print CSV') , '" />
			</div>
            </div>
            </form>';
	}
