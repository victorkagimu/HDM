...
if ((!isset($_POST['FromPeriod']) AND !isset($_POST['ToPeriod'])) OR isset($_POST['SelectADifferentPeriod'])){
	...
	$SQL = 'SELECT accountgroups.sectioninaccounts, 
			...
			Sum(CASE WHEN chartdetails.period=' . ($_POST['ToPeriod']+12) . ' THEN chartdetails.bfwdbudget + chartdetails.budget ELSE 0 END) AS yearbudgetcfwd
				...
				if ($myrow['parentgroupname']!=$ActGrp){
					while ($myrow['groupname']!=$ParentGroups[$Level] AND $Level>0) {
						...
						if ($Section == 1){ /*Income */	
							...
							$LeftOvers = $pdf->addTextWrap($Left_Margin+350,$YPos,70,$FontSize,number_format($GrpPrdActual[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format($GrpPrdBudget[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format($GrpYearBudget[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,number_format($GrpPrdLY[$Level]),'right');
							$YPos -= (2 * $line_height);
						} else { /*Costs */
							$LeftOvers = $pdf->addTextWrap($Left_Margin +($Level*10),$YPos,200 -($Level*10),$FontSize,$ActGrpLabel);
							$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format(-$GrpPrdActual[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format(-$GrpPrdBudget[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format(-$GrpYearBudget[$Level]),'right');
							$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,number_format(-$GrpPrdLY[$Level]),'right');
							$YPos -= (2 * $line_height);
...
// Print heading if at end of page
					...
					if ($Section == 1){ /*Income */	
						$LeftOvers = $pdf->addTextWrap($Left_Margin +($Level*10),$YPos,200 -($Level*10),$FontSize,$ActGrpLabel); $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format($GrpPrdActual[$Level]),'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format($GrpPrdBudget[$Level]),'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format($GrpYearBudget[$Level]),'right');
						$LeftOvers = $pdf->addTextWrap($Left_Margin+430,$YPos,70,$FontSize,number_format($GrpPrdLY[$Level]),'right');
						$YPos -= (2 * $line_height);
		...
		if ($myrow['sectioninaccounts'] != $Section){
			...
			if ($Section != ''){
				...
				if ($Section == 1) { /*Income*/
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format($SectionPrdActual),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format($SectionPrdBudget),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format($SectionYearBudget),'right');
				...
				if ($Section == 2){ /*Cost of Sales - need sub total for Gross Profit*/
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format($TotalIncome - $SectionPrdActual),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format($TotalBudgetIncome - $SectionPrdBudget),'right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format($TotalYearIncome - $SectionPrdBudget),'right');
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format($PrdGPPercent,1) . '%','right');
					$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format($BudgetGPPercent,1) . '%','right');
		...
		$AccountYearBudget = $myrow['yearbudgetcfwd'] - $myrow['firstprdbudgetbfwd'];
		$PeriodProfitLoss += $AccountPeriodActual;
		$PeriodBudgetProfitLoss += $AccountPeriodBudget;
		$YearBudgetProfitLoss += $AccountYearBudget;
		...
		for ($i=0;$i<=$Level;$i++){
			...
			$GrpYearBudget[$i] +=$AccountYearBudget;
		}
		...
		$SectionYearBudget +=$AccountYearBudget;
		...
		if ($_POST['Detail'] == _('Detailed')) {
			...
			if ($Section == 1) { /*Income*/
				$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format($AccountPeriodActual),'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format($AccountPeriodBudget),'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format($AccountYearBudget),'right');
	...
	if ($Section != ''){
		...
		if ($Section == 2){ /*Cost of Sales - need sub total for Gross Profit*/
			...
			$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format(100*($TotalIncome - $SectionPrdActual)/$TotalIncome,1) . '%','right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format(100*($TotalBudgetIncome - $SectionPrdBudget)/$TotalBudgetIncome,1) . '%','right');
			$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format(100*($TotalYearBudgetIncome - $SectionPrdBudget)/$TotalBudgetIncome,1) . '%','right');
	...
	$LeftOvers = $pdf->addTextWrap($Left_Margin+250,$YPos,70,$FontSize,number_format(-$PeriodProfitLoss),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+310,$YPos,70,$FontSize,number_format(-$PeriodBudgetProfitLoss),'right');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+370,$YPos,70,$FontSize,number_format(-$YearBudgetProfitLoss),'right');
	...
	$SQL = 'SELECT accountgroups.sectioninaccounts,
			...
			Sum(CASE WHEN chartdetails.period=' . $_POST['FromPeriod'] . ' THEN chartdetails.bfwdbudget ELSE 0 END) AS firstprdbudgetbfwd,
			Sum(CASE WHEN chartdetails.period=' . $_POST['ToPeriod'] . ' THEN chartdetails.bfwd + chartdetails.actual ELSE 0 END) AS lastprdcfwd,
			Sum(CASE WHEN chartdetails.period=' . ($_POST['ToPeriod']+12) . ' THEN chartdetails.bfwdbudget + chartdetails.budget ELSE 0 END) AS yearprdbudgetcfwd,
			Sum(CASE WHEN chartdetails.period=' . ($_POST['FromPeriod'] - 12) . ' THEN chartdetails.bfwd ELSE 0 END) AS lyfirstprdbfwd,
			Sum(CASE WHEN chartdetails.period=' . ($_POST['ToPeriod']-12) . ' THEN chartdetails.bfwd + chartdetails.actual ELSE 0 END) AS lylastprdcfwd,
			Sum(CASE WHEN chartdetails.period=' . $_POST['ToPeriod'] . ' THEN chartdetails.bfwdbudget + chartdetails.budget ELSE 0 END) AS lastprdbudgetcfwd
		FROM chartmaster INNER JOIN accountgroups
	...
	echo '<CENTER><FONT SIZE=4 COLOR=BLUE><B>' . _('Statement of Income and Expenditure for the'). ' ' . $NumberOfMonths . ' ' . _('months to'). ' ' . $PeriodToDate . '</B></FONT><BR>';
	...
	if ($_POST['Detail']=='Detailed'){
		$TableHeader = "<TR>
				...
				<TH COLSPAN=2>"._('Annual Budget')."</TH>
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
			...
			if ($myrow['parentgroupname']!=$ActGrp AND $ActGrp!=''){
				...
				if ($Section ==3){ /*Income */
						printf('<TR>
							...
							number_format($GrpYearBudget[$Level]),
		...
		if ($myrow['sectioninaccounts']!= $Section){
			...
			if ($SectionPrdLY+$SectionPrdActual+$SectionPrdBudget !=0){
				if ($Section==4) { /*Income*/
					...
					number_format($SectionYearBudget),
					number_format($SectionPrdLY));
					$TotalIncome = -$SectionPrdActual;
					$TotalBudgetIncome = -$SectionPrdBudget;
					$TotalYearBudgetIncome = -$SectionYearBudget;
					$TotalLYIncome = -$SectionPrdLY;
				} else {
					echo '<TR>
					<TD COLSPAN=2></TD>
				...
				if ($Section==2){ /*Cost of Sales - need sub total for Gross Profit*/
					...
					number_format($TotalYearBudgetIncome - $SectionYearBudget),
					...
					if ($TotalYearBudgetIncome !=0){
						$BudgetYearGPPercent = 100*($TotalYearBudgetIncome - $SectionYearBudget)/$TotalYearBudgetIncome;
					} else {
						$BudgetYearGPPercent =0;
					}
					if ($TotalLYIncome !=0){
						$LYGPPercent = 100*($TotalLYIncome - $SectionPrdLY)/$TotalLYIncome;
					} else {
					...
					printf('<TR>
						...
						number_format($BudgetYearGPPercent,1) . '%',
		...
		$AccountPeriodLY = $myrow['lylastprdcfwd'] - $myrow['lyfirstprdbfwd'];
		$AccountPeriodBudget = $myrow['lastprdbudgetcfwd'] - $myrow['firstprdbudgetbfwd'];
		$AccountYearBudget = $myrow['yearprdbudgetcfwd'] - $myrow['firstprdbudgetbfwd'];
		if ($Section==4) {
			$PeriodProfitLoss -= $AccountPeriodActual;
		} else {
			$PeriodProfitLoss -= $AccountPeriodActual;			
		...
		for ($i=0;$i<=$Level;$i++){
			...
			$GrpPrdBudget[$i] +=$AccountPeriodBudget;
			if (!isset($GrpYearBudget[$i])) {$GrpYearBudget[$i]=0;}
			$GrpYearBudget[$i] +=$AccountYearBudget;
		}
		$SectionPrdLY -=$AccountPeriodLY;
		$SectionPrdActual -=$AccountPeriodActual;
		...
		if ($_POST['Detail']==_('Detailed')){
			...
			if ($Section ==4){
				 printf('<td>%s</td>
					...
					number_format(-$AccountYearBudget),
	...
	if ($myrow['sectioninaccounts']!= $Section){
		...
		if ($Section==4) { /*Income*/
			...
			$TotalIncome = $SectionPrdActual;
			$TotalBudgetIncome = $SectionPrdBudget;
			$TotalYearBudgetIncome = $SectionPrdBudget;
			$TotalLYIncome = $SectionPrdLY;
		} else {
			echo '<TR>
			<TD COLSPAN=2></TD>
	...
	printf("<tr bgcolor='#ffffff'>
		...
		number_format($YearBudgetProfitLoss),
