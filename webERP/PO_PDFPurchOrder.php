	...
	$line_height=16;
	   ...
	   /*Yes there are line items to start the ball rolling with a page header */
		...
		while ($POLine=DB_fetch_array($result)){
			...
			if ($YPos-($line_height*3) <= $Bottom_Margin){
		        /* We reached the end of the page so finsih off the page and start a newy */
				$YPos -= $line_height*2;
				$LeftOvers = $pdf->addTextWrap($Left_Margin+1+64,$YPos,700,$FontSize,_('Signed by').'_________________________________________________________________________________', 'left');
				$PageNumber++;
				include ('includes/PO_PDFOrderPageHeader.inc');
...
//		$YPos -= $line_height*5;
		...
		if ($YPos-$line_height*1 <= $Bottom_Margin){ // need to ensure space for totals
			$PageNumber++;
			include ('includes/PO_PDFOrderPageHeader.inc');
		} //end if need a new page headed up
		$YPos = $Bottom_Margin + $line_height*3;
		$LeftOvers = $pdf->addTextWrap($Left_Margin+1+64,$YPos,700,$FontSize,_('Signed by').'_________________________________________________________________________________', 'left');
