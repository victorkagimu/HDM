	...
	if (isset($_POST['apportion'])) {
		for ($i=1; $i<=12; $i++) {
			...
			if ($_POST['AnnualAmountTY'] != '0') 
				$budget[$LastYearEndPeriod+($i)-1]	= $_POST['AnnualAmountTY']/12;
		}
	}
...
// Total Line
	...
	echo '<tr><td></td><td></td><td></td><td></td><td>'._('Annual Budget').'</td>';
	echo '<td><INPUT onChange="numberFormat(this,2)" onKeyPress="return restrictToNumbers(this, event)" TYPE=TEXT size=14 NAME="AnnualAmountTY" STYLE="text-align: right" value=0></td>';
	echo '</td><td></td><td>';
	echo '<td><INPUT onChange="numberFormat(this,2)" onKeyPress="return restrictToNumbers(this, event)" TYPE=TEXT size=14 NAME="AnnualAmount" STYLE="text-align: right" value=0></td>';
